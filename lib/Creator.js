...
class Creator {
    ...
    async create() {
            // 仓库信息 —— 模板信息
            let [social, repo] = await this.getRepoInfo();
            // 标签信息 —— 版本信息
            ...
            // 下载模板
            await this.download(repo, branch, social);

            if (social) {
                await overWrite(this.target);
            }
            // 模板使用提示
    ...
    async getRepoInfo() {
            ...
            let obj = await new inquirer.prompt([{
                ...
                type: "list",
                message: "选择模板",
                choices: [repo.name, 'Hooks-Admin'],
            }, ]);
            return [obj.repo !== repo.name, obj.repo];
        }
        // 获取版本信息及用户选择的版本
    async getBranch() {
        ...
        // 下载git仓库
    async download(repo, branch, social = false) {
        // 模板下载地址
        const templateUrl = !social ? `chapaofan-zy/${repo}#${branch}` : 'HalseySpicy/Hooks-Admin';
